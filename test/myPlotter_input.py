...
def makeresplot(hlist, algo, suffix, fileName, plotscaffold):
    ...
    quality = fileName.split(".")[0].split("_")[-1]
    ...
    for ich in range(4):
        ...
        #  print ich+1, iwh-3, ibin
            ...
            print("%s\tWh%s\tSt%s\t%s\t%s\t%s" % (
                quality,
                iwh - 3, ich + 1,
                hmatched[ich].GetBinContent(iwh),
                htotal[ich].GetBinContent(iwh),
                hmatched[ich].GetBinContent(iwh) / htotal[ich].GetBinContent(iwh))
            )
...
def combineresplots(hlist, legends, plottingStuff, path, savescaffold, fil):
    ...
    histo_dummy = r.TH1F("hEff_dummy", "", 20, -0.5, 19.5)
    histo_dummy.SetStats(False)
    histo_dummy.GetYaxis().SetRangeUser(plottingStuff['lowlimityaxis'], plottingStuff['highlimityaxis'])
    histo_dummy.GetYaxis().SetTitleOffset(plottingStuff['yaxistitleoffset'])
    histo_dummy.GetYaxis().SetTitle(plottingStuff['yaxistitle'])
    histo_dummy.GetXaxis().SetTitle(plottingStuff['xaxistitle'])
    histo_dummy.GetXaxis().SetNdivisions(120)
    histo_dummy.Draw()
    ...
    for ich in range(4):
        for iwh in range(-2, 3):
            histo_dummy.GetXaxis().ChangeLabel(ilabel, -1, -1, -1, -1, -1, (iwh > 0) * "+" + str(iwh))
            ilabel += 1

    histo_legs = []
    for iplot in range(len(hlist)):
        for i, ch in enumerate(chambTag):
            histo_legs.append(r.TH1F("hEff_%s_%s" % (iplot, ch), "", 5, -0.5 + 5 * i, 4.5 + 5 * i))
            histo_legs[-1].SetMarkerSize(plottingStuff['markersize'])
            histo_legs[-1].SetMarkerStyle(plottingStuff['markertypedir'][iplot])
            histo_legs[-1].SetMarkerColor(plottingStuff['markercolordir'][iplot])
            histo_legs[-1].SetLineColor(plottingStuff['markercolordir'][iplot])

            for iwh in range(-2, 3):
                histo_legs[-1].SetBinContent(iwh + 3, hlist[iplot].GetBinContent(5 * i + iwh + 3))

            if i == 0:
                leg.AddEntry(histo_legs[-1], legends[iplot], "P")
            histo_legs[-1].Draw("CP,hist,same")
